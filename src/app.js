...
const { getAllConfigurations } = require("./services/configurationService");
const { getVehicleClasses,getSingleTires } = require('./services/vehiclesService');
...
async function initializeControllers() {
  try {
    ...
    const controllers = configurations.map((config) => {
      if (config.controller_id === 1) {
        ...
        console.warn(`Unknown controller_id: ${config.controller_id} for station: ${config.station_name}`);
