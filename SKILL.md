...
3. **Enhancement:** `threeDimensionService` fetches vehicle dimensions in the background. (Pico wheel/tire classification was removed — see §6.)
4. **Recording:** `DataLogger` saves the vehicle details and axle records to the database immediately (axles/axles_after_allowance/flags are batch-inserted within one transaction). Image matching runs in the background; if the snapshot is not yet registered, `waitForImages` retries up to 3 times (fixed 500ms apart — `IMAGE_RETRY_COUNT`/`IMAGE_RETRY_DELAY_MS`) and then saves the record with missing images rather than losing it.
...
- **OCR:** Handled by `src/utils/ocrService.js`. It processes snapshots to extract license plate numbers. It supports receiving binary image buffers directly to bypass disk reads. Optional pre-processing via `OCR_PREPROCESS=1`. **Two-pass OCR** (`OCR_RETRY_ENHANCE`, on by default): if the raw image reads no plate, it retries automatically with an enhanced image (`ocr_recovered_by_enhance`). Diagnostic logs (`[OCR][raw]` full response, `[OCR][Crop]` coordinates) are gated behind `OCR_DEBUG=1` (off by default) — turn on to debug a plate crop that does not align.
- **LPR Burst Frames:** the LPR camera captures a burst of `LPR_BURST_FRAMES` frames/vehicle (default 2). The first frame is OCR'd in parallel with upload; only if it fails to read does the system OCR the remaining frames and pick the best (`pickBestPlate`). Normal vehicles (frame 1 reads) are not slowed. Metrics: `lpr_burst_captured`, `plate_recovered_by_extra_frame`, `burst_frames_identical` (watch — identical frames mean the camera returned a duplicate).
...
- **Cleanup:** `src/services/snapshotCleanupService.js` runs a **04:00 daily** job to delete snapshots (disk + `snapshots` table rows) older than **7 days**, oldest date-folder first.
- **Action:** Retention is now a **hardcoded constant** (`retentionDays = 7` in `snapshotCleanupService.js`) — it no longer reads the `configuration.retention_days` column. To change it you must edit that constant and restart (it is intentionally not runtime-adjustable). Log files (`logs/info-*.log`, `error-*.log`) are likewise kept 7 days via `maxFiles: "7d"` in `src/utils/logger.js`.
...
### 6. Pico Tire Classification — ⚠️ REMOVED
- **Status:** `src/services/picoService.js` and the `PICO_BASE` env var were **deleted** in the cleanup round. There is no Pico code in the repo anymore — do not reference it.
- **Where single/dual tire actually comes from today:** the `dual_tire` value on `axles` is set by `setSingleTire()` (`mapDataLogger.js`) using the `single_tires` DB table.
...
- **Image Wait Workflow:** Detailed in `src/controllers/DataLogger.js` (`waitForImages`). If snapshots are missing after the initial find, it retries up to 3 times (fixed 500ms apart — `IMAGE_RETRY_COUNT`/`IMAGE_RETRY_DELAY_MS`). There is no software on-demand capture.
